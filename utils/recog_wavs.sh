...
base=$(basename $wav_file_list)
decode_dir=${decode_dir}/${base}

if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    echo "stage 0: Data preparation $decode_dir $wav_file_list $base"

    [ -e "${decode_dir}/data" ] && rm -rf "${decode_dir}/data"
    mkdir -p "${decode_dir}/data"

    num_lines=$(wc -l < "${wav_file_list}")
    for idx in $(seq "${num_lines}"); do
        wav=$(sed -n "${idx}"p "${wav_file_list}")
        echo "${base}-${idx} sox $wav -R -r 16000 -c 1 -b 16 -t wav - dither |" >> ${decode_dir}/data/wav.scp
        echo "X ${base}-${idx}" >> ${decode_dir}/data/spk2utt
        echo "${base}-${idx} X" >> ${decode_dir}/data/utt2spk
        echo "${base}-${idx} X" >> ${decode_dir}/data/text
    done
